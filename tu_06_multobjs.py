...
class simpleMesh(meshWithRender):
...
class indicsRender_No_Normal(meshWithRender):
...
if __name__ == "__main__":
    ...
    win.add_mesh(indicsRender_No_Normal(meshName="resources/tu04/suzanne.obj",textureName="resources/tu04/uvmap.dds"))
    new_cube = []
    for idx in range(0,len(g_vertex_buffer_data),3):
        new_cube.append(g_vertex_buffer_data[idx]+3.0)
        new_cube.append(g_vertex_buffer_data[idx+1])
        new_cube.append(g_vertex_buffer_data[idx+2]-3.0)
    win.add_mesh(simpleMesh(new_cube, g_color_buffer_data))
    win.run()
